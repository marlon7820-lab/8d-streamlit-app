...
t = {
    "en": {
        "D1":"D1: Concern Details","D2":"D2: Similar Part Considerations","D3":"D3: Initial Analysis",
        ...
        "Detection_Why":"Detection Why","Systemic_Why":"Systemic Why","Save":"💾 Save 8D Report",
        "Download":"📥 Download XLSX","Training_Guidance":"Training Guidance","Example":"Example",
        "FMEA_Failure":"FMEA Failure Occurrence","Location":"Material Location","Status":"Activity Status",
        "Containment_Actions":"Containment Actions"
    },
    "es": {
        "D1":"D1: Detalles de la preocupación","D2":"D2: Consideraciones de partes similares","D3":"D3: Análisis inicial",
...
# ---------------------------
# Categories for D5 (example)
# ---------------------------
occurrence_categories = {"Human": ["Training", "Knowledge", "Error"], "Equipment": ["Tool failure", "Machine issue"], "Process": ["Process gap"]}
detection_categories = {"Inspection": ["Check missed", "Gauge incorrect"], "System": ["No alarm", "Software error"]}
systemic_categories = {"Management": ["Lack supervision", "Resource issue"], "Design": ["Spec unclear", "Design flaw"]}
...
for i, (step, note_dict, example_dict) in enumerate(npqp_steps):
    with tabs[i]:
        ...
        if step == "D4":
            st.session_state[step]["location"] = st.selectbox("Location of Material", ["", "Work in Progress", "Stores Stock", "Warehouse Stock", "Service Parts", "Other"], key="d4_location")
            st.session_state[step]["status"] = st.selectbox("Status of Activities", ["", "Pending", "In Progress", "Completed", "Other"], key="d4_status")
            ...
            st.text_area(f"{t[lang_key]['Root_Cause_Occ']}", value=suggest_root_cause(occ_whys) if occ_whys else "", height=80, disabled=True)
            st.text_area(f"{t[lang_key]['Root_Cause_Det']}", value=suggest_root_cause(det_whys) if det_whys else "", height=80, disabled=True)
            st.text_area(f"{t[lang_key]['Root_Cause_Sys']}", value=suggest_root_cause(sys_whys) if sys_whys else "", height=80, disabled=True)
        else:
...
det_whys = [w for w in st.session_state.d5_det_whys if w.strip()]
sys_whys = [w for w in st.session_state.d5_sys_whys if w.strip()]
occ_rc_text = suggest_root_cause(occ_whys) if occ_whys else ""
det_rc_text = suggest_root_cause(det_whys) if det_whys else ""
sys_rc_text = suggest_root_cause(sys_whys) if sys_whys else ""
