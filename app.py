...
if "D8" in step_index_map:
    with tabs[step_index_map["D8"]]:
        ...
        # ---------------------------
# PART 4: Excel export (memory-safe) and JSON backup/restore
# ---------------------------

# Build data rows from session_state consistently (keeps previous mapping)
def collect_data_rows():
    rows = []
    # D1-D3 basic answers
    for step, _, _ in npqp_steps[:3]:
        rows.append((step, st.session_state.get(step, {}).get("answer", ""), st.session_state.get(step, {}).get("extra", "")))

    # D4: location/status/actions
    d4_actions = st.session_state.get("D4_actions", "")
    d4_loc = st.session_state.get("D4_location", "")
    d4_status = st.session_state.get("D4_status", "")
    rows.append(("D4 - Material Location", d4_loc, ""))
    rows.append(("D4 - Activity Status", d4_status, ""))
    rows.append(("D4 - Containment Actions", d4_actions, ""))

    # D5 root causes + whys lists
    occ_whys = [w for w in st.session_state.get("d5_occ_whys", []) if w.strip()]
    det_whys = [w for w in st.session_state.get("d5_det_whys", []) if w.strip()]
    sys_whys = [w for w in st.session_state.get("d5_sys_whys", []) if w.strip()]

    rows.append(("D5 - Root Cause (Occurrence)", st.session_state.get("d5_occ_rc", ""), " | ".join(occ_whys)))
    rows.append(("D5 - Root Cause (Detection)", st.session_state.get("d5_det_rc", ""), " | ".join(det_whys)))
    rows.append(("D5 - Root Cause (Systemic)", st.session_state.get("d5_sys_rc", ""), " | ".join(sys_whys)))

    # D6 and D7 separated answers
    rows.append(("D6 - Corrective Actions (Occurrence)", st.session_state.get("D6_occ_answer",""), ""))
    rows.append(("D6 - Corrective Actions (Detection)", st.session_state.get("D6_det_answer",""), ""))
    rows.append(("D6 - Corrective Actions (Systemic)", st.session_state.get("D6_sys_answer",""), ""))

    rows.append(("D7 - Confirmation (Occurrence)", st.session_state.get("D7_occ_answer",""), ""))
    rows.append(("D7 - Confirmation (Detection)", st.session_state.get("D7_det_answer",""), ""))
    rows.append(("D7 - Confirmation (Systemic)", st.session_state.get("D7_sys_answer",""), ""))

    # D8
    rows.append(("D8 - Follow-up / Lessons Learned", st.session_state.get("D8_answer",""), ""))
...
def generate_workbook_bytes():
    ...
    ws.append([t[lang]['Report_Date'], st.session_state.get("report_date","")])
    ws.append([t[lang]['Prepared_By'], st.session_state.get("prepared_by","")])
    ws.append([])
    ...
    # Safe logo insertion: only if present and small-ish
    if os.path.exists(LOGO_PATH):
        try:
            with open(LOGO_PATH, "rb") as f:
                data = f.read()
            # create image from in-memory bytes
            ...
            # do not fail workbook creation for logo errors
            pass

    # Use save_virtual_workbook for memory-safe bytes output
    try:
        xlsx_bytes = save_virtual_workbook(wb)
    except Exception:
        # fallback to writing to BytesIO
        out = BytesIO()
        wb.save(out)
...
xlsx_bytes = None
try:
    xlsx_bytes = generate_workbook_bytes()
except Exception as e:
    st.error(f"Error generating Excel: {e}")

if xlsx_bytes:
    st.download_button(
        label=f"{t[lang]['Download']}",
        data=xlsx_bytes,
        file_name=f"8D_Report_{st.session_state.get('report_date','report').replace(' ','_')}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )

# ---------------------------
# JSON Backup / Restore (safe)
# ---------------------------
def make_serializable(v):
    """Try to make session_state values JSON serializable; fallback to string."""
    try:
        json.dumps(v)
        return v
    except Exception:
...
def generate_json_backup():
    ...
    for k, v in st.session_state.items():
        # skip internal Streamlit keys if any; keep everything else
        if k.startswith("_"):
            continue
        backup[k] = make_serializable(v)
...
with st.sidebar:
    ...
    st.download_button(
        ...
        file_name=f"8D_Report_Backup_{st.session_state.get('report_date','report').replace(' ','_')}.json",
        mime="application/json"
    )

    uploaded = st.file_uploader("Upload JSON to restore", type="json")
    if uploaded is not None:
        try:
            restore_data = json.load(uploaded)
            # update only keys that are expected to be in session_state to avoid unexpected shapes
            for k, v in restore_data.items():
                st.session_state[k] = v
            st.success("✅ Session restored from JSON. If UI doesn't refresh automatically, use the Reset button in Controls.")
            # do NOT call experimental_rerun blindly; user can reset if needed
        except Exception as e:
            st.error(f"Failed to restore JSON: {e}")
