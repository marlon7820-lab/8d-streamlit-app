...
tab_labels = []
for step, _, _ in npqp_steps:
    badge = "✅" if (st.session_state[step]["answer"].strip() or (step=="D5" and (any(w.strip() for w in st.session_state.d5_occ_whys) or any(w.strip() for w in st.session_state.d5_det_whys)))) else "❌"
...
for i, (step, note, example) in enumerate(npqp_steps):
    with tabs[i]:
        ...
        if step != "D5":
            ...
            st.session_state.D5["extra"] = st.text_area(f"{t[lang_key]['Root
